...
fun main(args: Array<String>) {
    ...
    val time = measureTimeMillis {
        coords = getCoordinatesForAdress(347991)
    }
    val fuu = aWalkInThePark(947991000)
    println("Coordinates: $coords ($fuu)Distance to (0, 0): ${getManhattanDistanceToCenter(
            coords)} It took ${time}ms to compute!")
...
class GridDataStructure {
    ...
    fun set(index: Coordinates, value: Int) {
        while (getIndexFromCoordinates(index) >= data.size) {
            ...
            println("Size of array is now: ${data.size}")
        }
